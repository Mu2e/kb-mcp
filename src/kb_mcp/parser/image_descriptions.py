...
def generate_image_descriptions(
    ...
    try:
        ...
        # Heads-up for known text-only models on the Fermilab proxy. These
        # accept image_url content without erroring but return a polite
        # refusal, so descriptions silently become garbage.
        # Vision capability re-verified 2026-04-27: qwen3.6 and qwen35-9b
        # both accept and respond to vision input — they were previously on
        # this list by mistake and have been removed.
        _TEXT_ONLY_MODELS = {
            "openai/gpt-oss-120b",
        ...
        if model in _TEXT_ONLY_MODELS:
            logger.warning(
                "PARSE_IMAGE_DESCRIPTION_MODEL=%s is text-only on the Fermilab "
                "proxy; image descriptions will be refusals, not real "
                "descriptions. Set PARSE_IMAGE_DESCRIPTION_MODEL to a "
                "vision-capable model (e.g. qwen/qwen3.6 [open, free, "
                "fastest], azure/claude-sonnet-4-6 [metered, best quality]).",
                model,
            )
