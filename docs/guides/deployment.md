...
Deployed as two `systemd --user` services on a Mu2e interactive node, with no
container — the package is installed from a pinned git ref into a versioned
venv.

That deployment has its own page, which is self-contained:
**[Mu2e Deployment](deploy-mu2e.md)**.
