...
There is no database password — the connection authenticates with GSSAPI from
the service account's ticket, in the cache the **service** reads.

A `systemd --user` service does not inherit the `KRB5CCNAME` of the shell that
installed it. It falls back to libkrb5's default `/tmp/krb5cc_<uid>`, while the
renewed ticket usually lives elsewhere, e.g. `/tmp/krb5cc_<uid>_auto`. Both
caches exist and hold the same principal, so the error is not "no credentials
cache" but `Ticket expired` — after a clean startup, on the first query only.
...
`kb-mcp-install-unit.sh --krb5-ccname "$KRB5CCNAME"` writes it into the unit.
Whatever renews that cache (typically a cron job holding a keytab) must keep
running, or the service fails the same way when the ticket lapses.

## 5. Check before enabling
...
```bash
# same three lines as step 1, with the new REF
...
| `... GSSAPI ... Ticket expired` | `KRB5CCNAME` — the service is reading a different cache than your shell. Step 4. |
| `permission denied for table <x>` / `CREATE TABLE` in the traceback | missing `USAGE` on the schema. Step 3. |
| `permission denied for table logs_search` | missing `INSERT`. Step 3. |
...
| `Could not import module 'PreTrainedModel'` | torchvision built against CUDA torch. Step 1. |
